...
// Deterministic compact money. ICU's `notation: "compact"` renders trailing
// zeros differently in Node vs the browser ($1.0K vs $1K), which trips React 19
// hydration and regenerates the whole tree — killing e.g. the WebGL car canvas.
// This is pure arithmetic, identical on server and client.
const compactMoney = (sym: string, n: number, signed = false): string => {
...
const COMPACT_MONEY: Record<string, (amount: number) => string> = {
  ...
  USD: (amount) => compactMoney("$", amount),
};
