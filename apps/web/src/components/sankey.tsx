...
const placeLabels = (
  ...
  for (const [, column] of columns) {
    ...
    for (const node of orderBy(column, [centerOf], ["asc"])) {
      ...
      const value = formatCompact(node.total);
...
const buildLayout = (
  ...
  return {
    ...placeLabels(nodes, nameMax, type, format, formatCompact),
    links,
    incident,
  };
};
...
// Manual compact currency — ICU's `notation: "compact"` renders trailing zeros
// differently in Node vs the browser ($386.00K vs $386K), which trips React 19
// hydration and regenerates the whole tree. This is deterministic on both.
const USD_COMPACT = (n: number) => {
  const a = Math.abs(n);
...
export function Sankey({
  ...
  expanded?: boolean;
  /** Which currency to format values in. A string, so it crosses the RSC boundary. */
  currency?: "THB" | "USD";
  /** Tooltip unit ("per month" for cash flow; "value" for a fund snapshot). */
  unitLabel?: string;
}) {
  const format = currency === "USD" ? USD_FULL : formatThb;
  const formatCompact = currency === "USD" ? USD_COMPACT : formatThbCompact;
  ...
  const { labelled, links, notes, incident } = useMemo(
    () => buildLayout(graph, expanded, type, format, formatCompact),
    [graph, expanded, type, format, formatCompact],
  );
  ...
  const tipRows = (): readonly TipRow[] => {
    if (hotLink !== undefined) {
      return [
        { key: "value", label: unitLabel, value: format(hotLink.value) },
      ];
    }
    if (hotNode === undefined) return [];
    const rows: TipRow[] = [
      { key: "total", label: unitLabel, value: format(hotNode.total) },
    ];
            ...
            {labelled.map((node) => {
              ...
              return (
                <rect
                  ...
                  tabIndex={0}
                  role="img"
                  aria-label={`${node.label}, ${format(node.total)} ${unitLabel}`}
