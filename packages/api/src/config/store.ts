...
/**
 * Fallback gateway from the environment, so the chat works out of the box when
 * `OPENAI_API_KEY` is set and nothing has been saved in-app yet. The in-app
 * config (below) always wins when present.
 */
const envGateway = (): GatewayConfig | undefined => {
...
export const readGateway = async (): Promise<GatewayConfig | undefined> => {
  ...
  if (!row.ok || row.value === undefined) return envGateway();
  return {
